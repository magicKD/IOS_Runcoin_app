...
@interface SNSellerViewController ()<UITextViewDelegate,UIScrollViewDelegate>{
...
@implementation SNSellerViewController
...
//    _explainLabel.text = @"添加图片不超过9张，文字备注不超过300字";
    ...
    _valueText.placeholder = @"请输入价格";
    _valueText.borderStyle = UITextBorderStyleRoundedRect;
//    _valueText.backgroundColor = [UIColor greenColor];
    ...
    [_submitBtn setBackgroundColor:[UIColor colorWithRed:169.0/255.0 green:28.0/255.0 blue:238.0/255.0 alpha:1.0]];
...
//    [_mianScrollView addSubview:_star];
//    [_mianScrollView addSubview:_explainLabel];
    [_mianScrollView addSubview:_valueText];
...
- (void)updateViewsFrame{
    ...
    _valueText.frame = CGRectMake(10, [self getPickerViewFrame].origin.y + [self getPickerViewFrame].size.height + 10, SCREENWIDTH - 20, 40);
    
//    //说明文字
//    _explainLabel.frame = CGRectMake(0, [self getPickerViewFrame].origin.y+[self getPickerViewFrame].size.height+10, SCREENWIDTH, 20);
//
//    //五星好评控件
//    _star.frame=CGRectMake((SCREENWIDTH-100)/2, CGRectGetMaxY(_explainLabel.frame)+10, 100, 20);
    
    //发布按钮
    _submitBtn.bounds = CGRectMake(10, _valueText.frame.origin.y+_valueText.frame.size.height +30, SCREENWIDTH -20, 40);
    _submitBtn.frame = CGRectMake(10, _valueText.frame.origin.y+_valueText.frame.size.height +30, SCREENWIDTH -20, 40);
...
- (void)submitToServer{
    ...
    _star.scroreBlock=^(NSNumber *number){
    ...
    SNUserModel * curUser = appDelegate.curUser;
    /*if(curUser==nil){
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:nil message:@"用户未登陆!" preferredStyle:UIAlertControllerStyleAlert];
    ...
    NSDictionary *parameters = @{
                                 ...
                                 @"publicKey": curUser.publicKey,
                                 @"amount":_textNumberLabel.text,
                                 @"info":_noteTextView.text,
                                 };
    */
    NSDictionary *parameters = @{
                                 @"address":@"0x1",
                                 @"publicKey": @"0x2",
                                 @"amount":_valueText.text,
                                 @"info":_noteTextView.text,
                                 };
    NSString *urlString = @"http://127.0.0.1:8080/cr";
    ...
    [manager POST:urlString parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        NSLog(@"hello");
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:nil message:@"发布   失败!" preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction *actionCacel = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            [self dismissViewControllerAnimated:YES completion:nil];
        }];
        [alertController addAction:actionCacel];
