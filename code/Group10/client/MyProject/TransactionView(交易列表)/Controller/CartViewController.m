...
- (void)viewDidLoad
{
    ...
    UINavigationItem *navItem = [[UINavigationItem alloc] initWithTitle:@"交易记录"];
    ...
    //added for testing
    SNGoodsDetail * good = [[SNGoodsDetail alloc] init];
    good.price = @"10";
    good.countOfNeed = 1;
    ...
    good.dateStr = strDate;
    delegate.arrayForCart = [[NSMutableArray alloc] init];
    [delegate.arrayForCart addObject:good];
    [delegate.arrayForCart addObject:good];
    [delegate.arrayForCart addObject:good];
...
- (void)setupBottomView
{
    ...
    [btn setTitle:@"交易确认" forState:UIControlStateNormal];
...
-(void)selectAllBtnClick:(UIButton*)button      //全选按钮事件
{
    ...
    if (isSelect)
    {
        for (SNGoodsDetail *model in delegate.arrayForCart)
        {
...
- (void)goPayBtnClick         //提交订单按钮事件  待写...
{
    ...
    {
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"友情提示" message:@"请至少选择一件商品再进行确认" preferredStyle:UIAlertControllerStyleAlert];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    SNCartTableCell *cell = [tableView dequeueReusableCellWithIdentifier:cellID];
    if (!cell)
    {
        cell = [[SNCartTableCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
    ...
    __block SNCartTableCell *wealCell = cell;
    cell.countAddBlock = ^()                            //增加数量的时候
    {
        ...
        SNGoodsDetail *model = [delegate.arrayForCart objectAtIndex:indexPath.row];
